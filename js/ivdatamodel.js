var IVDataModel = (function (groups, dimensions, points) {
    this.groups = groups;
    ...
    this.selectPoint = function(group, index) {
        ...
        if ((group != null) && (index != null)) {
            ...
            if (this.groups[group].reference) {
                ...
                this.nearestNeighbors = sortedAllNeighbors.slice(0, maxNearestNeighbors);
            }
        } else {
            sortedAllNeighbors = null;
            this.nearestNeighbors = [];
            ...
            if (thisArg.groups[group].reference) {
                ...
                for (var j = 0; j < pointsInGroup.length; j++, i++) {
                    ...
                    sortedAllNeighbors[i] = { 
                        "group": group, 
                        "index": j,
                        "dist": distance(thisArg, selectedPoint, point)
                    };
                }
            }
        }
    ...
    this.setMaxNearestNeighbors = function(value) {
        ...
        if ((selection.group != null) && (selection.index != null)) {
            if (this.groups[selection.group].reference) {
                this.nearestNeighbors = sortedAllNeighbors.slice(1, 1 + maxNearestNeighbors);
            } else {
                this.nearestNeighbors = sortedAllNeighbors.slice(0, maxNearestNeighbors);
            }
    ...
    var distance = function(thisArg, point1, point2) {
        var distance = 0;
        var dimension = null;
            
        for (var d = 0; d < activeDimensions; d++) {
            distance += Math.pow(thisArg.points[point1].loc[d] - thisArg.points[point2].loc[d], 2);
        }
    ...
    this.saveDataToBlob = function() {
        var tsv = "";  
       
        tsv += "popID\tindivID";
        for (var j = 0; j < this.dimensions.length / 2; j++) {
            tsv += "\t" + this.dimensions[j];
        }
        tsv += "\n";
        
        if ((selection.group != null) && (selection.index != null)) {
            var i = this.pointsByGroup[selection.group][selection.index];
            var entry = this.points[i].grp + "\t" +this.points[i].id;
            for (var j = 0; j < this.dimensions.length / 2; j++) {
                entry += "\t" + this.points[i].loc[j];
            }
            entry += "\n";
            tsv += entry;
            
            for (var k = 0; k < this.nearestNeighbors.length; k++) {
                i = this.pointsByGroup[this.nearestNeighbors[k].group][this.nearestNeighbors[k].index]
                entry = this.points[i].grp + "\t" +this.points[i].id;
                for (var j = 0; j < this.dimensions.length / 2; j++) {
                    entry += "\t" + this.points[i].loc[j];
                }
                entry += "\n";
            ...
            for (var i = 0; i < this.points.length; i++) {
                if (!this.activeGroups.hasOwnProperty(this.points[i].grp)) {
                    continue;
                }
                var entry = this.points[i].grp + "\t" +this.points[i].id;
        ...
        return new Blob([tsv], {type: "text/plain;charset=utf-8"})
    }
