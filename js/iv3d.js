var IV3D = (function (model, config) {
...
//    }       
    ...
    var symbols = new Array(config.symbols_3d.length);
    
    // Initialize symbols
    for (var i = 0; i < symbols.length; i++) {
        symbols[i] = new THREE.TextureLoader().load(config.symbols_3d[i]);
    }
...
//            console.log("append X = "  + append);
            while (append > 0) {
                if (Math.abs(dataViewCube.minX) < Math.abs(dataViewCube.maxX)) {
                    dataViewCube.minX = dataViewCube.minX - 1;
                } else {
                    dataViewCube.maxX = dataViewCube.maxX + 1;
                }
                append--;
            }       
...
//            console.log("append Y = "  + append);
            while (append > 0) {
                if (Math.abs(dataViewCube.minY) < Math.abs(dataViewCube.maxY)) {
                    dataViewCube.minY = dataViewCube.minY - 1;
                } else {
                    dataViewCube.maxY = dataViewCube.maxY+ 1;
                }
                append--;
            }     
...
//            console.log("append Z = "  + append);
            while (append > 0) {
                if (Math.abs(dataViewCube.minZ) < Math.abs(dataViewCube.maxZ)) {
                    dataViewCube.minZ = dataViewCube.minZ - 1;
                } else {
                    dataViewCube.maxZ = dataViewCube.maxZ + 1;
                }
                append--;
            }     
...
//        sprite.scale.set(40, 20, 1);
    ...
    function drawAxes() {
        ...
        var ticksSize = dataViewCube.sideSize / 100; // 100 is my magic number
        
        ticks_geometry.vertices.push(
            new THREE.Vector3(dataViewCube.minX - ticksSize, dataViewCube.maxY, dataViewCube.maxZ), 
            new THREE.Vector3(dataViewCube.minX, dataViewCube.maxY, dataViewCube.maxZ));
        ticks_geometry.vertices.push(
            new THREE.Vector3(dataViewCube.minX - ticksSize, dataViewCube.minY, dataViewCube.maxZ), 
            new THREE.Vector3(dataViewCube.minX, dataViewCube.minY, dataViewCube.maxZ));
        ticks_geometry.vertices.push(
            new THREE.Vector3(dataViewCube.minX, dataViewCube.minY, dataViewCube.maxZ), 
            new THREE.Vector3(dataViewCube.minX, dataViewCube.minY, dataViewCube.maxZ + ticksSize));
        ticks_geometry.vertices.push(
            new THREE.Vector3(dataViewCube.maxX, dataViewCube.minY, dataViewCube.maxZ), 
            new THREE.Vector3(dataViewCube.maxX + ticksSize, dataViewCube.minY, dataViewCube.maxZ));
        ticks_geometry.vertices.push(
            new THREE.Vector3(dataViewCube.maxX, dataViewCube.minY, dataViewCube.maxZ), 
            new THREE.Vector3(dataViewCube.maxX, dataViewCube.minY, dataViewCube.maxZ + ticksSize));
        ticks_geometry.vertices.push(
            new THREE.Vector3(dataViewCube.maxX, dataViewCube.minY, dataViewCube.minZ), 
            new THREE.Vector3(dataViewCube.maxX + ticksSize, dataViewCube.minY, dataViewCube.minZ));
        ...
        labelX.sprite.position.set(dataViewCube.centerX, dataViewCube.minY, dataViewCube.maxZ + ticksSize * 2);
        labelY.sprite.position.set(dataViewCube.minX, dataViewCube.centerY, dataViewCube.maxZ + ticksSize * 2);
        labelZ.sprite.position.set(dataViewCube.maxX + ticksSize * 2, dataViewCube.minY, dataViewCube.centerZ);
        ...
        labelTickMinX.sprite.position.set(dataViewCube.minX, dataViewCube.minY, dataViewCube.maxZ + ticksSize * 2);
        labelTickMaxX.sprite.position.set(dataViewCube.maxX, dataViewCube.minY, dataViewCube.maxZ + ticksSize * 2);
        labelTickMinY.sprite.position.set(dataViewCube.minX - ticksSize * 2, dataViewCube.minY, dataViewCube.maxZ);
        labelTickMaxY.sprite.position.set(dataViewCube.minX - ticksSize * 2, dataViewCube.maxY, dataViewCube.maxZ);
        labelTickMinZ.sprite.position.set(dataViewCube.maxX + ticksSize * 2, dataViewCube.minY, dataViewCube.minZ);
        labelTickMaxZ.sprite.position.set(dataViewCube.maxX + ticksSize * 2, dataViewCube.minY, dataViewCube.maxZ);
...
//        mouse2d.y = (-(d3.event.y - boundingClientRect.top) / canvas.height) * 2 * window.devicePixelRatio + 1;
    ...
    var updatePicked = function() {
        ...
        raycaster.params.Points.threshold = scale;
    ...
    var modelToScreenXYZ = function(point) {
        var vector = new THREE.Vector3(point.loc[config.xDim], point.loc[config.yDim], point.loc[config.zDim]);
    ...
    var updateTooltip = function() {
        if ((picked.group != tooltip.group) || (picked.index != tooltip.index)) {
            ...
            if ((picked.group != null) && (picked.index != null)) {
                ...
                var coordinate = modelToScreenXYZ(point);
    ...
    var initializeGL = function() {
        ...
        camera = new THREE.PerspectiveCamera(45, canvas.width / canvas.height, 0.001, dataViewCube.sideSize * 20);
        ...
        if (dataBoundingBox.height < 1 || dataBoundingBox.width < 1 || dataBoundingBox.length < 1) {
            var minSide = Math.min(dataBoundingBox.height, dataBoundingBox.width, dataBoundingBox.length);
            raycaster.params.Points.threshold = minSide / 2; // Set sensitivity assuming that points are scattered uniformly along smallest side and avg. pairwise distance is half of this side.
        } else {
